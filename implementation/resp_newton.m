function [disp_row, disp_col,ind, max_response] = resp_newton(response, responsef, iterations, ky, kx, use_sz)
...
for num = 1:size(max_response, 3)
    disp_row(num) = (mod(max_pos_y(1,1,num) + pi, 2*pi) - pi) / (2*pi) * use_sz(1);
    disp_col(num) = (mod(max_pos_x(1,1,num) + pi, 2*pi) - pi) / (2*pi) * use_sz(2);
end
end